                ...
                <li>
                    ...
                    <div class="text-product-cart-header">
                        <h2> <span> {{ $item->product_name }}</span>
                            <button class="remove-cart-header" data-url="{{ route('site.cart.destroy', $item->id) }}"> <i
                                    class="bi bi-trash"></i></button>
                        </h2>
                        <div>
                            <span> {{ $item->quantity }}</span>
                            <div class="price-cart-header"> EGP {{ $item->price }} <span class="old-price">
                                    {{ $item->products->price_after_discount ?? 0 }}
                                    EGP </span></div>
                        </div>
                    </div>
                </li>
    ...
    @if ($orderItems->orderItems->count() > 0)
        <div class="btns-cart-header">
            <div class="total-cart-header pb-3">
                <h2> {{ transWord('المجموع') }} <span> EGP {{ $orderItems->price_before_discount }}
                    </span>
                </h2>
            </div>
